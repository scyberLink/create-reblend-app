# Enforce props alphabetical sorting (`reblend/jsx-sort-props`)
...
```js
...
"reblend/jsx-sort-props": [<enabled>, {
...
When `reservedFirst` is defined, Reblend reserved props (`children`, `dangerouslySetInnerHTML` - **only for DOM components**, `key`, and `ref`) must be listed before all other props, but still respecting the alphabetical order:

```jsx
<Hello key={0} ref={johnRef} name="John">
  <div
    dangerouslySetInnerHTML={{ __html: 'ESLint Plugin Reblend!' }}
    ref={dangerDivRef}
  />
</Hello>
```

If given as an array, the array's values will override the default list of reserved props. **Note**: the values in the array may only be a **subset** of Reblend reserved props.
