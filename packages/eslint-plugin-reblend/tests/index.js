...
describe('configurations', () => {
  it('should export a ‘recommended’ configuration', () => {
    ...
    Object.keys(plugin.configs[configName].rules).forEach(ruleName => {
      assert.ok(ruleName.startsWith('reblend/'));
      const subRuleName = ruleName.slice('reblend/'.length);
      assert(plugin.rules[subRuleName]);
    });

    ruleFiles.forEach(ruleName => {
      const inRecommendedConfig =
        !!plugin.configs[configName].rules[`reblend/${ruleName}`];
  ...
  it('should export an ‘all’ configuration', () => {
    ...
    ruleFiles.forEach(ruleName => {
      const inDeprecatedRules = Boolean(plugin.deprecatedRules[ruleName]);
      const inConfig =
        typeof plugin.configs[configName].rules[`reblend/${ruleName}`] !==
        'undefined';
