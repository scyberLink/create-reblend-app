...
module.exports = {
  meta: {
    docs: {
      description:
        'Disallow missing displayName in a Reblend component definition',
  ...
  create: Components.detect((context, components, utils) => {
    ...
    /**
     * Checks if Reblend.forwardRef is nested inside Reblend.memo
     * @param {ASTNode} node The AST node being checked.
     * @returns {Boolean} True if Reblend.forwardRef is nested inside Reblend.memo, false if not.
     */
    function isNestedMemo(node) {
      const argumentIsCallExpression =
        node.arguments &&
        node.arguments[0] &&
    ...
    function reportMissingDisplayName(component) {
      if (
        testReblendVersion(context, '^0.14.10 || ^15.7.0 || >= 16.12.0') &&
    ...
    return {
      ...
      MemberExpression(node) {
        ...
        markDisplayNameAsDeclared(
          component.node.type === 'TSAsExpression'
      ...
      CallExpression(node) {
        ...
        if (
          ...
          // Skip over Reblend.forwardRef declarations that are embedded within
          // a Reblend.memo i.e. Reblend.memo(Reblend.forwardRef(/* ... */))
          // This means that we raise a single error for the call to Reblend.memo
          // instead of one for Reblend.memo and one for Reblend.forwardRef
