...
function isCreateCloneElement(node, context) {
  ...
  if (node.type === 'Identifier') {
    ...
    if (variable && variable.type === 'ImportSpecifier') {
      return variable.parent.source.value === 'reblend';
    }
  }
...
module.exports = {
  ...
  create(context) {
    ...
    function isUsingReblendChildren(node) {
      const callee = node.callee;
      if (!callee || !callee.property || !callee.object) {
        return null;
      }

      const isReblendChildMethod =
        ['map', 'forEach'].indexOf(callee.property.name) > -1;
      if (!isReblendChildMethod) {
        return null;
      }
    ...
    function getMapIndexParamName(node) {
      ...
      const callbackArg = isUsingReblendChildren(node)
        ? node.arguments[1]
        : node.arguments[0];
