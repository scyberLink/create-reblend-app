...
const messages = {
  lifecycle:
    '{{propertyName}} is a Reblend lifecycle method, and should not be an arrow function or in a class field. Use an instance method instead.',
};
                ...
                isBlockBody
                  ...
                  : fixer.replaceTextRange(
                      ...
                      `{ return ${previousComment
                        .map(x => getText(context, x))
                        .join('')}${getText(context, body)}${nextComment
                        .map(x => getText(context, x))
                        .join('')}; }`
                    )
              );
