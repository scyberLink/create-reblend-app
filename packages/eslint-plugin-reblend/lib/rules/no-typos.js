...
const messages = {
  ...
  noReblendBinding: "`'reblend'` imported without a local `Reblend` binding.",
};
...
module.exports = {
  ...
  create: Components.detect((context, components, utils) => {
    ...
    function checkValidPropType(node) {
      if (
        node.name &&
        !PROP_TYPES.some(propTypeName => propTypeName === node.name)
      ) {
    ...
    function isPropTypesPackage(node) {
      return (
        ...
        (node.type === 'MemberExpression' &&
          ...
          node.object.name === reblendPackageName)
      );
    }
    ...
    function checkValidProp(node) {
      if ((!propTypesPackageName && !reblendPackageName) || !node) {
        return;
      }
    ...
    function reportErrorIfLifecycleMethodCasingTypo(node) {
      ...
      if (
        ...
        (node.computed && typeof nodeKeyName !== 'string')
      ) {
        return;
      }
      ...
      lifecycleMethods?.static?.forEach(method => {
        if (
          ...
          report(
            context,
            messages.staticLifecycleMethod,
      ...
      lifecycleMethods.instance
        .concat(lifecycleMethods.static)
        .forEach(method => {
          if (
            nodeKeyName &&
    ...
    return {
      ImportDeclaration(node) {
        ...
        } else if (node.source && node.source.value === 'reblend') {
          // import { PropTypes } from "reblend"
          if (node.specifiers.length > 0) {
            reblendPackageName = node.specifiers[0].local.name; // guard against accidental anonymous `import "reblend"`
          } else {
            report(context, messages.noReblendBinding, 'noReblendBinding', {
              node,
            });
          }
      ...
      'ClassProperty, PropertyDefinition'(node) {
        if (
          !node.static ||
          !componentUtil.isES6Component(node.parent.parent, context)
        ) {
          return;
        }
      ...
      MemberExpression(node) {
        ...
        if (
          ...
          reportErrorIfPropertyCasingTypo(
            node.parent.right,
            node.property,
            true
          );
