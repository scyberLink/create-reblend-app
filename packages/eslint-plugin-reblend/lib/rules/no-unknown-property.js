...
const ATTRIBUTE_TAGS_MAP = {
  ...
  noModule: ['script'],
  // Media events allowed only on audio and video tags, see https://github.com/facebook/reblend/blob/256aefbea1449869620fb26f6ec695536ab453f5/CHANGELOG.md#notable-enhancements
  onAbort: ['audio', 'video'],
  onCancel: ['dialog'],
...
const DOM_PROPERTY_NAMES_TWO_WORDS = [
  ...
  // Reblend specific attributes https://reblendjs.org/docs/dom-elements.html#differences-in-attributes
  'className',
...
function getDOMPropertyNames(context) {
  const ALL_DOM_PROPERTY_NAMES = DOM_PROPERTY_NAMES_TWO_WORDS.concat(
    DOM_PROPERTY_NAMES_ONE_WORD
  );
  // this was removed in Reblend v16.1+, see https://github.com/facebook/reblend/pull/10823
  if (!testReblendVersion(context, '>= 16.1.0')) {
    return ALL_DOM_PROPERTY_NAMES.concat('allowTransparency');
  }
  // these were added in Reblend v16.4.0, see https://reblendjs.org/blog/2018/05/23/reblend-v-16-4.html and https://github.com/facebook/reblend/pull/12507
  if (testReblendVersion(context, '>= 16.4.0')) {
 ...
 *
 * Note - these exclusions are not made by Reblend core team, but `eslint-plugin-reblend` community.
 *
 ...
 * if the name starts with "data-" and has alphanumeric words (browsers require lowercase, but Reblend and TS lowercase them),
...
const messages = {
  ...
  dataLowercaseRequired:
    "Reblend does not recognize data-* props with uppercase characters on a DOM element. Found '{{name}}', use '{{lowerCaseName}}' instead",
};
  ...
  create(context) {
    ...
    return {
      JSXAttribute(node) {
        ...
        // Let's dive deeper into tags that are HTML/DOM elements (`<button>`), and not Reblend components (`<Button />`)
