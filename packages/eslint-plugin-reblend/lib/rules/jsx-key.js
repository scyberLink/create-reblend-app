...
const messages = {
  ...
  missingIterKeyUsePrag:
    'Missing "key" prop for element in iterator. Shorthand fragment syntax does not support providing keys. Use {{reblendPrag}}.{{fragPrag}} instead',
  missingArrayKey: 'Missing "key" prop for element in array',
  missingArrayKeyUsePrag:
    'Missing "key" prop for element in array. Shorthand fragment syntax does not support providing keys. Use {{reblendPrag}}.{{fragPrag}} instead',
  keyBeforeSpread:
    '`key` prop must be placed before any `{...spread}, to avoid conflicting with Reblend’s new JSX transform: https://reblendjs.org/blog/2020/09/22/introducing-the-new-jsx-transform.html`',
  ...
  create(context) {
    ...
    function checkArrowFunctionWithJSX(node) {
      ...
      if (node.body.type === 'ConditionalExpression') {
        ...
        shouldCheckNode(node.body.right)
      ) {
    ...
    const childrenToArraySelector = `:matches(
      CallExpression
        [callee.object.object.name=${reblendPragma}]
