...
module.exports = {
  ...
  create: Components.detect((context, components, utils) => {
    ...
    const ignoreClassFields =
      (context.options[1] && context.options[1].ignoreClassFields === true) ||
      false;
    const destructureInSignature =
      (context.options[1] && context.options[1].destructureInSignature) ||
      'ignore';
    ...
    function handleStatelessComponent(node) {
      ...
      if (
        ...
        report(
          context,
          messages.noDestructPropsInSFCArg,
          'noDestructPropsInSFCArg',
          {
            node,
          }
        ...
        report(
          context,
          messages.noDestructContextInSFCArg,
          'noDestructContextInSFCArg',
          {
            node,
          }
    ...
    function handleSFCUsage(node) {
      ...
      if (isPropUsed && configuration === 'always' && !node.optional) {
        report(
          context,
          messages.useDestructAssignment,
          'useDestructAssignment',
          {
            node,
            data: {
    ...
    function isInClassProperty(node) {
      ...
      while (curNode) {
        if (
          curNode.type === 'ClassProperty' ||
          curNode.type === 'PropertyDefinition'
        ) {
          return true;
        }
    ...
    return {
      ...
      VariableDeclarator(node) {
        ...
        // let {foo} = props;
        const destructuringSFC =
          destructuring &&
          (node.init.name === 'props' || node.init.name === 'context');
        // let {foo} = this.props;
        const destructuringClass =
          destructuring &&
          node.init.object &&
