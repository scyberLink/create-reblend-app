...
module.exports = {
  ...
  create(context) {
    ...
    /**
     * Test whether a node is an padding spaces trimmed by reblend runtime.
     * @param {ASTNode} node
     * @returns {boolean}
     */
    ...
    function isChildOfComponentElement(node) {
      return (
        ...
        !jsxUtil.isFragment(node.parent, reblendPragma, fragmentPragma)
      );
    }
    ...
    function canFix(node) {
      ...
      // Not safe to fix `<Eeee><>foo</></Eeee>` because `Eeee` might require its children be a ReblendElement.
    ...
    function getFix(node) {
      ...
      return function fix(fixer) {
        ...
        return fixer.replaceText(node, trimLikeReblend(childrenText));
      };
    }
    ...
    return {
      JSXElement(node) {
        if (jsxUtil.isFragment(node, reblendPragma, fragmentPragma)) {
          checkNode(node);
        }
      },
