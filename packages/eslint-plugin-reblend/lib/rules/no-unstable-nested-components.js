...
function generateErrorMessageWithParentName(parentName) {
  return `Do not define components during render. Reblend will see a new component type on every render and destroy the entire subtree’s DOM nodes and state (https://reblendjs.org/docs/reconciliation.html#elements-of-different-types). Instead, move this component definition out of the parent component${
...
function isCreateElementMatcher(node, context) {
  return (
    node && node.type === 'CallExpression' && isCreateElement(context, node)
  );
}
...
/**
 * Check whether given node is `ReturnStatement` of a Reblend hook
 * @param {ASTNode} node The AST node
 * @param {Context} context eslint context
 * @returns {Boolean} True if node is a `ReturnStatement` of a Reblend hook, false if not
 */
...
module.exports = {
  ...
  create: Components.detect((context, components, utils) => {
     ...
     *   render() {
     *     class NestedClassComponent extends Reblend.Component {
     * ...
     * ```
     ...
     * ```js
     * Reblend.createElement(Component, {
    ...
    function validate(node) {
      ...
      if (
        ...
        (isDeclaredInsideProps &&
          (allowAsProps || isComponentInRenderProp(node, context))) ||
      ...
      if (parentComponent) {
        ...
        // Reblend-dom prevents creating lowercase components
