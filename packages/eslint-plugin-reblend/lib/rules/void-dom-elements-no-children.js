...
module.exports = {
  ...
  create: context => ({
    ...
    CallExpression(node) {
      ...
      if (args.length < 1) {
        // Reblend.createElement() should not crash linter
        return;
      }
      ...
      if (firstChild) {
        // e.g. Reblend.createElement('br', undefined, 'Foo')
        report(context, noChildrenInVoidEl, 'noChildrenInVoidEl', {
          node,
          data: {
            element: elementName,
      ...
      if (hasChildrenPropOrDanger) {
        // e.g. Reblend.createElement('br', { children: 'Foo' })
        report(context, noChildrenInVoidEl, 'noChildrenInVoidEl', {
          node,
          data: {
            element: elementName,
