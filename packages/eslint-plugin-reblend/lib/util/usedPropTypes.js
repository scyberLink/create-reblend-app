...
const testReblendVersion = require('./version').testReblendVersion;
...
/**
 * Returns true if the given node is a Reblend Component lifecycle method
 * @param {ASTNode} node The AST node being checked.
 * @param {boolean} checkAsyncSafeLifeCycles
...
/**
 * Returns true if the given node is inside a Reblend Component lifecycle
 * method.
 * @param {ASTNode} node The AST node being checked.
 * @param {boolean} checkAsyncSafeLifeCycles
...
module.exports = function usedPropTypesInstructions(
  ...
  const checkAsyncSafeLifeCycles = testReblendVersion(context, '>= 16.3.0');
  ...
  return {
    ...
    'MemberExpression, OptionalMemberExpression'(node) {
      ...
      const propVariable = propVariables.get(
        ast.unwrapTSAsExpression(node.object).name
      );
      if (propVariable) {
