...
/**
 * Check if the node is explicitly declared as a descendant of a Reblend Component
 * @param {any} node
 * @param {Context} context
 * @returns {boolean}
 */
function isExplicitComponent(node, context) {
  ...
  const relevantTags = commentAst.tags.filter(tag => tag.name === 'Reblend');
...
function isES6Component(node, context) {
  ...
  if (node.superClass.type === 'MemberExpression') {
    return (
      ...
      /^Reblend$/.test(node.superClass.property.name)
    );
  }
  if (node.superClass.type === 'Identifier') {
    return /^Reblend$/.test(node.superClass.name);
  }
  return false;
}
...
/**
 * Checks if a component extends Reblend
 * @param {ASTNode} node
 * @param {Context} context
 * @returns {boolean}
 */
function isPureComponent(node, context) {
  const pragma = getPragma(context);
  if (node.superClass) {
    return new RegExp(`^${pragma}$`).test(getText(context, node.superClass));
  }
  return false;
}
