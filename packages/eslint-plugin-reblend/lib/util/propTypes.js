...
module.exports = function propTypesInstructions(context, components, utils) {
  ...
  const genericReblendTypesImport = new Set();
  // import { FC as X } from 'reblend' -> localToImportedMap = { x: FC }
  ...
  /**
   * Checks if prop should be validated by plugin-reblend-proptypes
  ...
  const typeDeclarationBuilders = {
    ...
    ObjectTypeAnnotation(annotation, parentName, seen) {
      ...
      const containsIndexers = Boolean(
        annotation.indexers && annotation.indexers.length
      );
  ...
  /**
   * Creates the representation of the Reblend props type annotation for the component.
  ...
  /**
   * Creates the representation of the Reblend propTypes for the component.
   * The representation is used to verify nested used properties.
   ...
   *    the property is declared without the need for further analysis.
   */
  function buildReblendDeclarationTypes(value, parentName, rootNode) {
    if (
      value &&
      value.callee &&
      value.callee.object &&
    ...
    if (
      ...
      switch (callName) {
          ...
          iterateProperties(
            ...
            (childKey, childValue, propNode) => {
              if (childValue) {
                ...
                const fullName = [parentName, childKey].join('.');
                const types = buildReblendDeclarationTypes(
                  childValue,
                  fullName,
                  rootNode
                );
        ...
        case 'objectOf': {
          const fullName = [parentName, '*'].join('.');
          const child = buildReblendDeclarationTypes(
            argument,
            fullName,
            rootNode
          );
        ...
        case 'oneOfType': {
          ...
          const unionTypeDefinition = {
            ...
            children: argument.elements.map(element =>
              buildReblendDeclarationTypes(element, parentName, rootNode)
            ),
          };
          if (unionTypeDefinition.children.length === 0) {
            // no complex type found, simply accept everything
  ...
  function isValidReblendGenericTypeAnnotation(annotation) {
    if (annotation.typeName) {
      if (annotation.typeName.name) {
        // if FC<Props>
        const typeName = annotation.typeName.name;
        if (!genericReblendTypesImport.has(typeName)) {
          return false;
        }
        ...
        if (
          !genericReblendTypesImport.has(left) ||
  ...
  /**
   * Returns the left most typeName of a node, e.g: FC<Props>, Reblend.FC<Props>
   * The representation is used to verify nested used properties.
   * @param {ASTNode} node
    ...
    searchDeclarationByName(node) {
      ...
      if (astUtil.isTSTypeReference(node)) {
        ...
        const shouldTraverseTypeParams =
          genericReblendTypesImport.has(leftMostName);
        const nodeTypeParams = node.typeParameters;
        if (
          shouldTraverseTypeParams &&
          nodeTypeParams &&
          nodeTypeParams.length !== 0
        ) {
          // All reblend Generic types are derived from:
          // type PropsWithChildren<P> = P & { children?: ReblendNode | undefined }
  ...
  function markPropTypesAsDeclared(node, propTypes, rootNode) {
    ...
    switch (propTypes && propTypes.type) {
      ...
      case 'ObjectExpression':
        iterateProperties(
          ...
          (key, value, propNode) => {
            ...
            const types = buildReblendDeclarationTypes(value, key, rootNode);
            types.fullName = key;
            types.name = key;
            types.node = propNode;
      ...
      case 'Identifier': {
        const firstMatchingVariable = variableUtil
  ...
  function markAnnotatedFunctionArgumentsAsDeclared(node, rootNode) {
    ...
    if (
      ...
      (node.parent.callee.name === 'forwardRef' ||
        (node.parent.callee.object &&
          ...
          node.parent.callee.object.name === 'Reblend' &&
    ...
    if (isNodeAnnotated) {
      ...
      if (!isValidReblendGenericTypeAnnotation(annotation)) return;
  ...
  function resolveSuperParameterPropsType(node) {
    ...
    try {
      ...
      propsParameterPosition =
        node.superTypeParameters.params.length <= 2 ? 0 : 1;
    }
  ...
  return {
    ...
    ImportDeclaration(node) {
      // parse `import ... from 'reblend`
      if (node.source.value === 'reblend') {
        node.specifiers.forEach(specifier => {
          if (
            // handles import * as X from 'reblend'
            specifier.type === 'ImportNamespaceSpecifier' ||
            // handles import Reblend from 'reblend'
            specifier.type === 'ImportDefaultSpecifier'
          ) {
            genericReblendTypesImport.add(specifier.local.name);
          }

          // handles import { FC } from 'reblend' or import { FC as X } from 'reblend'
          if (
