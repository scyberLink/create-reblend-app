  ...
  if (variableInScope) {
    ...
    if (latestDef) {
      ...
      if (latestDef.node.type === 'VariableDeclarator' && latestDef.node.init) {
        ...
        // get "require('reblend')" from: "{variable} = require('reblend')"
        if (latestDef.node.init.type === 'CallExpression') {
          requireExpression = latestDef.node.init;
        }
        // get "require('reblend')" from: "variable = require('reblend').variable"
        if (
          !requireExpression &&
          latestDef.node.init.type === 'MemberExpression' &&
      ...
      // latest definition is an import declaration: import {<variable>} from 'reblend'
      if (
        latestDef.parent &&
