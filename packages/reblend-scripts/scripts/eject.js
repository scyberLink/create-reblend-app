...
console.log(
  chalk.cyan.bold(
    'NOTE: Create Reblend App 2+ supports TypeScript, Sass, CSS Modules and more without ejecting: ' +
      'https://reblendjs.org/blog/2018/10/01/create-reblend-app-v2.html'
  )
);
console.log();
...
prompts({
  ...
  if (appPackage.devDependencies) {
    // We used to put reblend-scripts in devDependencies
  ...
  if (fs.existsSync(paths.appTypeDeclarations)) {
    try {
      ...
      // Remove reblend-scripts reference since they're getting a copy of the types in their project
      content =
        content
          // Remove reblend-scripts types
          .replace(
            /^\s*\/\/\/\s*<reference\s+types.+?"reblend-scripts".*\/>.*(?:\n|$)/gm,
            ''
          )
          .trim() + os.EOL;
  ...
  if (ownPath.indexOf(appPath) === 0) {
    try {
      // remove reblend-scripts and reblend-scripts binaries from app node_modules
      Object.keys(ownPackage.bin).forEach(binKey => {
  ...
  if (fs.existsSync(paths.yarnLockFile)) {
    ...
    if (process.platform === 'win32') {
      // https://github.com/scyberLink/create-reblend-app/pull/3806#issuecomment-357781035
      // Yarn is diligent about cleaning up after itself, but this causes the reblend-scripts.cmd file
