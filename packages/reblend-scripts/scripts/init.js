...
function tryGitCommit(appPath) {
  try {
    ...
    execSync('git commit -m "Initialize project using Create Reblend App"', {
      stdio: 'ignore',
    });
    return true;
  } catch (e) {
...
module.exports = function (
  ...
  if (!templateName) {
    ...
    console.error(
      `You can fix this by running ${chalk.cyan(
        'npm uninstall -g create-reblend-app'
      )} or ${chalk.cyan(
        'yarn global remove create-reblend-app'
      )} before using ${chalk.cyan('create-reblend-app')} again.`
    );
    return;
  }
  ...
  if (templateJson.dependencies || templateJson.scripts) {
    ...
    console.log(
      chalk.red(
        'Root-level `dependencies` and `scripts` keys in `template.json` were deprecated for Create Reblend App 5.\n' +
