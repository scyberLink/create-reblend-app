...
console.log(
  chalk.cyan.bold(
    'NOTE: Create Scansio App 2+ supports TypeScript, Sass, CSS Modules and more without ejecting: ' +
      'https://scansiojs.org/blog/2018/10/01/create-scansio-app-v2.html'
  )
);
console.log();
...
prompts({
  ...
  if (appPackage.devDependencies) {
    // We used to put scansio-scripts in devDependencies
  ...
  if (fs.existsSync(paths.appTypeDeclarations)) {
    try {
      ...
      // Remove scansio-scripts reference since they're getting a copy of the types in their project
      content =
        content
          // Remove scansio-scripts types
          .replace(
            /^\s*\/\/\/\s*<reference\s+types.+?"scansio-scripts".*\/>.*(?:\n|$)/gm,
            ''
          )
          .trim() + os.EOL;
  ...
  if (ownPath.indexOf(appPath) === 0) {
    try {
      // remove scansio-scripts and scansio-scripts binaries from app node_modules
      Object.keys(ownPackage.bin).forEach(binKey => {
  ...
  if (fs.existsSync(paths.yarnLockFile)) {
    ...
    if (process.platform === 'win32') {
      // https://github.com/scyberLink/create-scansio-app/pull/3806#issuecomment-357781035
      // Yarn is diligent about cleaning up after itself, but this causes the scansio-scripts.cmd file
