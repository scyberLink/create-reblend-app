...
const ReblendHooksESLintPlugin = require('eslint-plugin-react-hooks');
const BabelEslintParser = require('@babel/eslint-parser');
const ReblendHooksESLintRule = ReblendHooksESLintPlugin.rules['rules-of-hooks'];
  ...
  valid: [
    ...
    },
    {
      code: normalizeIndent`
        ...
        // to forwardRef.
        const FancyButton = Reblend.forwardRef((props, ref) => {
          useHook();
    ...
    },
    {
      code: normalizeIndent`
        ...
        // forwardRef.
        const FancyButton = Reblend.forwardRef(function (props, ref) {
          useHook();
          return <button {...props} ref={ref} />
        });
      `,
    ...
    },
    {
      code: normalizeIndent`
        // Valid because hooks can be used in anonymous function arguments to
        // Reblend.memo.
        const MemoizedFunction = Reblend.memo(props => {
          useHook();
          return <button {...props} />
        });
      `,
    ...
    },
    {
      code: normalizeIndent`
        ...
        Reblend.unknownFunction((foo, bar) => {
          if (foo) {
            useNotAHook(bar)
          }
        });
    ...
    {
      code: normalizeIndent`
        import * as Reblend from 'react';
        function App() {
          if (shouldShowText) {
            const text = use(query);
            const data = Reblend.use(thing);
  ...
  invalid: [
    ...
    },
    {
      code: normalizeIndent`
        // Invalid because it's a common misunderstanding.
        // We *could* make it valid but the runtime error could be confusing.
        const ComponentWithHookInsideCallback = Reblend.forwardRef((props, ref) => {
          useEffect(() => {
            useHookInsideCallback();
          });
          return <button {...props} ref={ref} />
    ...
    },
    {
      code: normalizeIndent`
        // Invalid because it's a common misunderstanding.
        // We *could* make it valid but the runtime error could be confusing.
        const ComponentWithHookInsideCallback = Reblend.memo(props => {
          useEffect(() => {
            useHookInsideCallback();
          });
          return <button {...props} />
    ...
    },
    {
      code: normalizeIndent`
        ...
        Reblend.unknownFunction(function notAComponent(foo, bar) {
    ...
    },
    {
      code: normalizeIndent`
        ...
        useState();
        if (foo) {
          const foo = Reblend.useCallback(() => {});
        }
        useCustomHook();
      `,
      errors: [
        topLevelError('useState'),
        topLevelError('Reblend.useCallback'),
    ...
    {
      code: normalizeIndent`
        class ClassComponentWithFeatureFlag extends Reblend.Component {
          render() {
            if (foo) {
              useFeatureFlag();
            }
    ...
    {
      code: normalizeIndent`
        class ClassComponentWithHook extends Reblend.Component {
          render() {
            Reblend.useState();
          }
        }
      `,
      errors: [classError('Reblend.useState')],
    },
    {
      code: normalizeIndent`
...
function conditionalError(hook, hasPreviousFinalizer = false) {
  return {
    message:
      `Reblend Hook "${hook}" is called conditionally. Reblend Hooks must be ` +
      'called in the exact same order in every component render.' +
      (hasPreviousFinalizer
        ? ' Did you accidentally call a Reblend Hook after an early return?'
        : ''),
  };
}
...
function loopError(hook) {
  return {
    message:
      `Reblend Hook "${hook}" may be executed more than once. Possibly ` +
      'because it is called in a loop. Reblend Hooks must be called in the ' +
...
function functionError(hook, fn) {
  return {
    message:
      `Reblend Hook "${hook}" is called in function "${fn}" that is neither ` +
      'a Reblend function component nor a custom Reblend Hook function.' +
      ' Reblend component names must start with an uppercase letter.' +
      ' Reblend Hook names must start with the word "use".',
  };
}
...
function genericError(hook) {
  return {
    message:
      `Reblend Hook "${hook}" cannot be called inside a callback. Reblend Hooks ` +
      'must be called in a Reblend function component or a custom Reblend ' +
      'Hook function.',
  };
}
...
function topLevelError(hook) {
  return {
    message:
      `Reblend Hook "${hook}" cannot be called at the top level. Reblend Hooks ` +
      'must be called in a Reblend function component or a custom Reblend ' +
      'Hook function.',
  };
}
...
function classError(hook) {
  return {
    message:
      `Reblend Hook "${hook}" cannot be called in a class component. Reblend Hooks ` +
      'must be called in a Reblend function component or a custom Reblend ' +
      'Hook function.',
  };
}
...
function useEffectEventError(fn) {
  return {
    message:
      `\`${fn}\` is a function created with Reblend Hook "useEffectEvent", and can only be called from ` +
...
function asyncComponentHookError(fn) {
  return {
    message: `Reblend Hook "${fn}" cannot be called in an async function.`,
  };
}
...
describe('rules-of-hooks/rules-of-hooks', () => {
  ...
  }).run('eslint: v7', ReblendHooksESLintRule, tests);
  ...
  }).run('eslint: v9', ReblendHooksESLintRule, tests);
});
