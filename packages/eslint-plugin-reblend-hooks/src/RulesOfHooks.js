...
/**
 * Checks if the node is a Reblend component name. Reblend component names must
...
function isReblendFunction(node, functionName) {
  return (
    node.name === functionName ||
    (node.type === 'MemberExpression' &&
      node.object.name === 'Reblend' &&
...
function isForwardRefCallback(node) {
  return !!(
    node.parent &&
    node.parent.callee &&
    isReblendFunction(node.parent.callee, 'forwardRef')
  );
}

/**
 * Checks if the node is a callback argument of Reblend.memo. This anonymous
...
function isMemoCallback(node) {
  return !!(
    node.parent &&
    node.parent.callee &&
    isReblendFunction(node.parent.callee, 'memo')
  );
}
...
module.exports = {
  ...
  create(context) {
    let lastEffect = null;
    const codePathReblendHooksMapStack = [];
    ...
    return {
      ...
      onCodePathStart: () => codePathReblendHooksMapStack.push(new Map()),

      // Process our code path.
      //
      // Everything is ok if all Reblend Hooks are both reachable from the initial
      // segment and reachable from every final segment.
      onCodePathEnd(codePath, codePathNode) {
        const reactHooksMap = codePathReblendHooksMapStack.pop();
        ...
        // will want to error since neither are Reblend function components or
        // hook functions - unless it is an anonymous function argument to
        // forwardRef or memo.
        const codePathFunctionName = getFunctionName(codePathNode);

        // This is a valid code path for Reblend hooks if we are directly in a Reblend
        ...
        // Make sure all Reblend Hooks pass our lint invariants. Log warnings
        // if not.
          ...
          for (const hook of reactHooks) {
            ...
            if (cycled && !isUseIdentifier(hook)) {
              context.report({
                node: hook,
                message:
                  `Reblend Hook "${getSource(hook)}" may be executed ` +
                  'more than once. Possibly because it is called in a loop. ' +
                  'Reblend Hooks must be called in the exact same order in ' +
                  'every component render.',
              });
            }

            // If this is not a valid code path for Reblend hooks then we need to
            ...
            if (isDirectlyInsideComponentOrHook) {
              ...
              if (isAsyncFunction) {
                context.report({
                  node: hook,
                  message:
                    `Reblend Hook "${getSource(hook)}" cannot be ` +
              ...
              if (
                ...
                const message =
                  `Reblend Hook "${getSource(hook)}" is called ` +
                  'conditionally. Reblend Hooks must be called in the exact ' +
                  'same order in every component render.' +
                  (possiblyHasEarlyReturn
                    ? ' Did you accidentally call a Reblend Hook after an' +
                      ' early return?'
                    : '');
              ...
              const message =
                `Reblend Hook "${getSource(hook)}" cannot be called ` +
                'in a class component. Reblend Hooks must be called in a ' +
                'Reblend function component or a custom Reblend Hook function.';
              context.report({ node: hook, message });
            } else if (codePathFunctionName) {
              // Custom message if we found an invalid function name.
              const message =
                `Reblend Hook "${getSource(hook)}" is called in ` +
                `function "${getSource(codePathFunctionName)}" ` +
                'that is neither a Reblend function component nor a custom ' +
                'Reblend Hook function.' +
              ...
              const message =
                `Reblend Hook "${getSource(hook)}" cannot be called ` +
                'at the top level. Reblend Hooks must be called in a ' +
                'Reblend function component or a custom Reblend Hook function.';
              context.report({ node: hook, message });
            } else {
              ...
              if (isSomewhereInsideComponentOrHook && !isUseIdentifier(hook)) {
                const message =
                  `Reblend Hook "${getSource(hook)}" cannot be called ` +
                  'inside a callback. Reblend Hooks must be called in a ' +
                  'Reblend function component or a custom Reblend Hook function.';
                context.report({ node: hook, message });
              }
            }
      ...
      CallExpression(node) {
        if (isHook(node.callee)) {
          ...
          const reactHooksMap = last(codePathReblendHooksMapStack);
        ...
        if (
          ...
          context.report({
            ...
            message:
              `\`${getSource(
                node
              )}\` is a function created with Reblend Hook "useEffectEvent", and can only be called from ` +
