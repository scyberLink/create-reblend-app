...
export default {
  ...
  create(context) {
    ...
    function visitFunctionWithDependencies(
      ...
      //
      // According to the rules of Reblend you can't read a mutable value in pure
      ...
      // const [state, setState] = useState() / Reblend.useState()
      //               ^^^ true for this reference
      // const [state, dispatch] = useReducer() / Reblend.useReducer()
      //               ^^^ true for this reference
      // const ref = useRef()
      //       ^^^ true for this reference
      ...
      function isStableKnownHookValue(resolved) {
        ...
        // Step into `= Reblend.something` initializer.
        if (
          callee.type === 'MemberExpression' &&
          callee.object.name === 'Reblend' &&
      ...
      currentRefsInEffectCleanup.forEach(
        ({ reference, dependencyNode }, dependency) => {
          ...
          // Is Reblend managing this ref or us?
          ...
          // We only want to warn about Reblend-managed refs.
          ...
          reportProblem({
            ...
            message:
              ...
              `this ref points to a node rendered by Reblend, copy ` +
      ...
      function reportStaleAssignment(writeExpr, key) {
        ...
        reportProblem({
          node: writeExpr,
          message:
            `Assignments to the '${key}' variable from inside Reblend Hook ` +
      ...
      if (!declaredDependenciesNode) {
        ...
        if (setStateInsideEffectWithoutDeps) {
          ...
          reportProblem({
            node: reactiveHook,
            message:
              `Reblend Hook ${reactiveHookName} contains a call to '${setStateInsideEffectWithoutDeps}'. ` +
      ...
      if (!isArrayExpression && !isTSAsArrayExpression) {
        ...
        reportProblem({
          ...
          message:
            `Reblend Hook ${getSource(reactiveHook)} was passed a ` +
        ...
        arrayExpression.elements.forEach(declaredDependencyNode => {
          ...
          if (declaredDependencyNode.type === 'SpreadElement') {
            reportProblem({
              ...
              message:
                `Reblend Hook ${getSource(reactiveHook)} has a spread ` +
            ...
            if (/Unsupported node type/.test(error.message)) {
              if (declaredDependencyNode.type === 'Literal') {
                ...
                reportProblem({
                  node: declaredDependencyNode,
                  message:
                    `Reblend Hook ${getSource(reactiveHook)} has a ` +
      ...
      reportProblem({
        node: declaredDependenciesNode,
        message:
          `Reblend Hook ${getSource(reactiveHook)} has ` +
    ...
    function visitCallExpression(node) {
      const callbackIndex = getReblendiveHookCallbackIndex(
        node.callee,
        options
      );
      if (callbackIndex === -1) {
        // Not a Reblend Hook call that needs deps.
        return;
      }
      const callback = node.arguments[callbackIndex];
      const reactiveHook = node.callee;
      const reactiveHookName =
        getNodeWithoutReblendNamespace(reactiveHook).name;
      ...
      // then the hook will not work and Reblend will throw a TypeError.
      // So no need to check for dependency inclusion.
      if (!callback) {
        reportProblem({
          node: reactiveHook,
          message:
            `Reblend Hook ${reactiveHookName} requires an effect callback. ` +
      ...
      if (!declaredDependenciesNode && !isEffect) {
        ...
        if (
          ...
          reportProblem({
            node: reactiveHook,
            message:
              `Reblend Hook ${reactiveHookName} does nothing when called with ` +
      ...
      switch (callback.type) {
        ...
        default:
          ...
          reportProblem({
            node: reactiveHook,
            message:
              `Reblend Hook ${reactiveHookName} received a function whose dependencies ` +
      ...
      reportProblem({
        node: reactiveHook,
        message:
          `Reblend Hook ${reactiveHookName} has a missing dependency: '${callback.name}'. ` +
...
function getNodeWithoutReblendNamespace(node, options) {
  if (
    node.type === 'MemberExpression' &&
...
function getReblendiveHookCallbackIndex(calleeNode, options) {
  const node = getNodeWithoutReblendNamespace(calleeNode);
